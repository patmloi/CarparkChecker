...
async def login(
    formData: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(getDb)
):
    """2. Login"""
    ...
    token = createToken(id=user.id, expiresDelta=tokenExpires)
...
async def readCarparkAvail(
    currentUser: User = Depends(getCurrentActiveUser),
    dateTime: Optional[datetime] = Form(None),
):
